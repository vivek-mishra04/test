...
if (isset ($_POST['username'])){
	...
	$roll = preg_replace('#[^A-Za-z0-9]#i', '', $_POST['roll']); // filter everything but letters and numbers
	 ...
	 // Database duplicate roll check setup for use below in the error handling if else conditionals
	 $sql_roll_check = mysql_query("SELECT USER_ID FROM myMembers WHERE USER_ID='$roll'"); 
     ...
     if ((!$username)|| (!$roll) || (!$gender) || (!$b_m) || (!$b_d) || (!$b_y) || (!$email1) || (!$email2) || (!$pass1) || (!$pass2)) { 
     ...
     $sql = mysql_query("INSERT INTO myMembers (username, USER_ID, gender, birthday, email, password, ipaddress, sign_up_date) 
     VALUES('$username','$roll','$gender','$full_birthday','$email1','$db_password', '$ipaddress', now())")  
      or die (mysql_error());
	 
	 $id = mysql_insert_id();
	 
	 // Create directory(folder) to hold each user's files(pics, MP3s, etc.)		
     mkdir("../profiles/members/$id", 0755);	

	 
	 $sql = mysql_query("INSERT INTO user (USERNAME, USER_ID, gender, birthday, email, PASSWORD, ipaddress, sign_up_date) 
     VALUES('$username','$roll','$gender','$full_birthday','$email1','$db_password', '$ipaddress', now())") 
...
$(document).ready(function() {
	$("#username").blur(function() {
		$("#nameresponse1").removeClass().text('Checking Username...').fadeIn(1000);
		$.post("scripts/check_signup_name.php",{ username:$(this).val() } ,function(data) {
		  	$("#nameresponse1").fadeTo(200,0.1,function() { 
...
$(document).ready(function() {
	$("#roll").blur(function() {
		$("#nameresponse2").removeClass().text('Checking Roll No...').fadeIn(1000);
		$.post("scripts/check_signup_rollno.php",{ roll:$(this).val() } ,function(data) {
		  	$("#nameresponse2").fadeTo(200,0.1,function() { 
			  $(this).html(data).fadeTo(900,1);	
			});
        });
	});
...
<table class="mainBodyTable" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="738" valign="top">
        ...
        <h2 style="margin-left:80px; color:#FFF">Create Your Account </h2>
        ...
        <form action="register.php" method="post" enctype="multipart/form-data">
          ...
          <tr>
            ...
            <td width="452" bgcolor="#FFFFFF"><input name="username" type="text" class="formFields" id="username" value="<?php print "$username"; ?>" size="32" maxlength="20" />
              <span id="nameresponse1"><span class="textSize_9px"><span class="greyColor">Alphanumeric Characters Only</span></span></span></td>
          </tr>
          <tr>
          <td width="114" bgcolor="#FFFFFF">Roll Number:<span class="brightRed"> *</span></td>
            <td width="452" bgcolor="#FFFFFF"><input name="roll" type="text" class="formFields" id="roll" value="<?php print "$roll"; ?>" size="32" maxlength="20" />
              <span id="nameresponse2"><span class="textSize_9px"><span class="greyColor"> Enter your Current Roll number</span></span></span></td>
          </tr>
